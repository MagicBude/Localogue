...
2026-09-11 Desktop 长页面导航按 JavBoss 的常驻操作原则完成适配：Work / Person 详情的返回按钮常驻顶栏，作品与人物浏览的分页同时出现在吸顶筛选区和结果底部。用户在长列表或详情任意位置都能返回或翻页，翻页后从新结果开头继续浏览。当前等待用户实机验收滚动、分页和逐级返回手感。
