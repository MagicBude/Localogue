...
2026-09-18 内容目录新增流程改为一次明确操作：用户点击“添加并扫描”并选择目录后，保存配置并立即只扫描该目录；不引入后台监控，也不重扫其他已配置目录。已有目录继续支持单独扫描和全部扫描，等待 Windows 实机验收目录选择、保存与任务衔接。

2026-09-18 扫描主链进入适度性能与可观察性收口：NFO 文本读取/XML 解析采用固定 6 路并发，目录发现、NFO 解析/保存、图片保存和视频分析复用目录卡片实时进度；同番号多媒体继续作为 Work → MediaFile 一对多正常状态。等待大目录实机验收吞吐和进度刷新。
