...
2026-09-18 Desktop 一级信息架构开始收敛：侧栏改为工作台、资料库、扫描与处理、设置四个主要任务入口，作品/人物/分类/收藏以及扫描审核/资料问题/历史改为上下文页签；现有页面能力和查询逻辑保持不变，等待 Windows 实机确认导航理解成本。

2026-09-18 工作台统计改为 Native 轻量摘要：SQLite 优先读取作品、人物、系列和媒体绑定列的 ID/COUNT，旧 JSON 资料库只解析必要字段；移除首页全量作品/人物/素材加载与最近卡片计算。自动检查通过，等待 Windows 实机确认旧 Runtime 更新后的工作台统计和空库状态。

2026-09-18 移除侧栏底部资料库摘要卡片，将影片库切换和新建入口移到窗口顶栏；侧栏收起后不再因为资料库切换而被迫展开，顶栏选择器保持固定位置。等待 Windows 实机确认多资料库切换和窄窗口布局。
