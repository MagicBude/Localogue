...
- 优化影片库切换入口：移除侧栏底部占空间的资料库卡片，将切换控件和“新建影片库”入口移到窗口顶栏；侧栏现在只承载导航和折叠控制。
- 工作台统计改用 Native 轻量摘要查询：SQLite 优先读取集合 ID / 媒体绑定列，旧 JSON 资料库只解析必要字段；首页不再加载十万条作品、人物、素材和词表来显示几个数字。
- 收敛 Desktop 一级信息架构：侧栏改为“工作台、资料库、扫描与处理、设置”，作品、人物、分类浏览和收藏统一进入资料库上下文页签；审核、资料问题和历史统一进入扫描与处理，关于保留为低频入口。
- 统一 Desktop 二级页签与筛选浮窗的按钮基础组件：页签、筛选入口、关闭和完成动作复用 `UiButton`，保留各自布局样式与键盘语义，后续交互不再重复手写按钮状态。
- 修正无边框窗口标题栏交互：改用 Tauri 官方拖动区域标记，交互控件保持 no-drag，由原生标题栏区域处理拖动和双击放大/还原，避免 React 手写拖动逻辑拦截双击。
- 修复人物列表点击后白屏：滚动位置恢复逻辑改为遵守 React Hook 固定调用顺序；窗口顶栏接入 Tauri 原生双击放大/还原，输入框、语言按钮和窗口控制区保持独立交互。
- 作品和人物从列表打开详情时改为覆盖式详情窗口：原列表保持挂载，关闭窗口即可回到原筛选、页码和滚动位置；作品与人物之间的关联跳转支持逐层关闭，不再依赖返回上一页按钮。
- 修正 Desktop 资料库设置布局：内容目录操作按钮固定在卡片右侧，设置分类标签滚动时吸顶；作品和人物详情返回会恢复原筛选、页码、每页数量与滚动位置，不再回到列表开头。
- 收敛 Desktop 资料库信息架构：新建或切换影片库后，在“影片库与目录”内直接查看、添加、移除内容目录、调整 NFO/图片/视频扫描范围并启动扫描；“扫描任务”改为进度、历史和高级排查入口。移除未内置 Web 服务的浏览器按钮，并将诊断入口统一到“关于”。
- 媒体绑定候选增加发行日期、时长和已关联媒体数量，并提供“查看信息”展开区；直接显示 Work 的标题、原文标题、简介等文字资料，不离开绑定弹窗，绑定仍需单独确认。
- 修正来源分类提示与一批真实 NFO 词条：`4小时+ / 4时间以上作品 / 无毛 / 学生妹 / 职业装 / 乳液` 精确路由到已有 Work Type 或 Genre，促销标题进入 Source-only；`淫乱真实 / 爱美臀 / 强迫口交` 标为需要人工审核，不再误报为普通未映射。媒体“管理绑定”改为居中宽对话框，不再把操作面板插到长页面底部。
- Desktop 扫描工作台改为跨页面常驻：切换到作品、人物、设置等页面只隐藏扫描界面，不再卸载任务协调器、进度轮询和历史记录流程；返回目录页可继续查看同一任务。设置页的数据存储位置增加“打开文件夹”，复用受限 Native 文件管理器入口。
- Desktop 新建影片库流程补齐首个内容目录：点击新建后先选择内容目录，再准备独立数据位置并直接进入 NFO → 图片 → 视频扫描；已经创建但没有内容目录的空资料库可在设置页一键补选并开始扫描，不再要求跳转到目录页二次配置。
- Desktop 统一扫描增加轻量实时进度：目录发现、NFO 解析、NFO 保存、本地图片保存和视频分析共用状态条；NFO 读取与 XML 解析改为固定 6 路受控并发，Canonical 写入继续顺序执行，避免大目录串行过慢或无上限并发造成资源压力。
- 修复日期、番号和标题无分隔符拼接时的番号污染：`2025-01-31SAME-151标题.mp4` 现在识别为 `SAME-151`，不会误认成 `31SAME-151`；媒体身份验证新增真实无空格文件名回归案例。
- 作品、人物和分类浏览统一使用共享筛选浮窗；分类页的使用情况与题材分组移入顶部筛选入口，窄窗口统一切换右侧抽屉。人物状态、年份和罩杯选项增加 self-excluding 结果计数。
- 人物筛选补充出生地、罩杯，以及人物图片、出生日期、身高、三围和简介的有无条件；条件进入共享 PersonQuery/queryPeople，并支持已选 Chips。人物详情把编辑改为首屏按钮与独立对话框，核心事实并入头部以减少正文占高。
- 对齐 115-Desktop 的浏览底栏与筛选交互：左侧固定显示结果总数，仅在超过一页时显示分页和跳页，右侧每页数量保持固定；作品和人物共用单一筛选按钮与锚点浮窗，替代分散筛选入口和展开式人物筛选区。
- 继续参考 115-Desktop 统一 Desktop 浏览工作区：作品、人物和分类页使用独立工具条、单一结果框和结果底栏；分类页取消多层卡片阴影，改为连续分组与边界分隔，题材维度条固定在结果框顶部，三类页面复用统一空状态。
- Desktop 人物页筛选与作品页统一为顶部工具栏：常用搜索 / 状态 / 排序常驻，年份和身高收进“更多”弹层，已选条件可单独移除，分页与清除移到结果行；同时修复本轮发现的 Desktop Webview 构建阻塞。
- Desktop 作品与人物浏览开始采用媒体优先密度：压缩顶部应用栏、页面标题区、筛选条和卡片间距，释放首屏给海报/封面与人物图片；查询、分页、筛选和详情入口保持不变。
- 参考 115-Desktop 重排浏览框架：全局搜索移到窗口级顶部栏，作品与人物结果底部新增页码按钮、跳转页和每页数量选择；瀑布流继续使用连续加载。

- Desktop 内容来源由三组重叠路径收敛为带“视频 / NFO / 图片”范围开关的统一内容目录；旧路径字段在迁移期自动合并并继续生成兼容镜像，旧设置和回退版本不会立即失效。
- 设置页删除“额外媒体目录”和“额外 NFO / 图片目录”两套重复入口，同一目录现在只出现一次；扫描编排根据目录范围分别取得有效根目录。
- Desktop 普通界面统一为“影片库、内容目录、社区资料、待审核资料、个人备份”五个核心概念；设置、侧栏、首页、收藏、社区资料包、作品/人物编辑、导入提示和帮助教程移除不必要的 Canonical / Evidence / Shared Pack 等内部术语，并同步日英翻译。
- 新增用户用语规范，明确“影片库”是独立个人收藏，“内容目录”保存原始文件，“社区资料”是只读公共元数据，“个人备份”不包含原始视频；内部 Domain、Schema 和 SQLite 表名不随 UI 改名。
- Desktop 首次设置在用户选择内容目录后直接进入“导入与整理”并启动统一同步，复用既有 NFO → 图片 → Media 编排和可观察进度，不再跳回首页要求二次点击。
- Desktop 按目录同步卡片增加运行状态、上次扫描时间、耗时、新增、更新、关联和未关联统计；单目录同步只高亮本轮对应目录，失败信息留在目录卡片中。
- Desktop 统一同步完成后在原卡片提供“查看作品库”和“处理未关联媒体”，未关联入口原地定位到媒体列表，形成明确的下一步操作。
- Desktop 普通入口将“同步资料库”统一改为“扫描资料库”，明确同时检查 NFO、图片和视频；视频文件独立检查改为默认折叠的高级操作，减少两个相近按钮造成的困惑。
- 产品文档统一为“本地优先的 AV 个人资料库与策展管理器”，正式入口收敛到 Desktop，当前阶段进入 0.2 Desktop Beta Readiness；README、快速上手、范围和路线图不再把已完成的 SQLite 迁移写成未来工作。
- 新增两项提议 ADR：分阶段冻结并最终退役 Web 产品壳，以及通过受控子进程 Adapter 将在线元数据保存为 Evidence。两项尚未接受，不代表在线刮削或 Web 删除已经实现。

- V2 Storage Migration 完成技术收口：数据概念统一为 Source Evidence、Curated Catalog 与 Personal Library；Desktop 使用只读 `catalog.db` 和可写 `local.db`，JSON/CSV 保留为交换、审核与恢复格式。迁移文档补齐构建、验证、运行时门控、旧 Pack 回退和 SQLite → JSON 回导流程。
...
- Desktop 右侧作品筛选板支持拖动标题区自由移动，并将坐标限制在当前窗口可见范围内；筛选、清除、关闭与 WorkQuery 更新语义不变。
- 根据实机反馈重做 Desktop 作品筛选：高级条件改为固定在窗口右侧的 410px 单列侧板，头部、滚动内容与底部操作明确分区；同时将占据整行的“新建作品”卡片压缩成按钮，新建表单改用弹窗，让作品在首屏更早出现。
- Desktop 作品详情把“编辑作品”提升到标题、收藏与评分所在的首屏操作区；编辑改为独立宽弹窗，按基础信息、简介、关系与分类分组，保存继续复用 Private Override、引用校验与删除审计链。
- Desktop 作品筛选改为紧凑常驻工具栏加临时高级筛选浮层：排序、筛选入口、视图和分页保持一行，日期、时长、收藏、评分与多维 Facet 在浮层内即时生效；已选条件继续由数量徽标和 Chips 呈现，关闭浮层不会清除条件。
- Desktop 清除剩余 8 处 `window.confirm`，Work / Person / Asset 删除、孤儿图片清理、分类修复、审核提交和历史恢复统一使用应用内确认框；原 Repository、Snapshot、回滚与 Native 引用检查保持不变。
- Desktop 长页面导航根据实机反馈再次收敛：分页只保留吸顶入口；详情返回改为独立右下角悬浮按钮，不再压在资料库分类上方，也不在正文重复显示。
- 顶部应用框架删除重复的设置入口和语义含糊的手动“刷新资料”；新增居中的作品全局搜索与紧凑语言菜单，保留界面语言 / 元数据语言独立能力。

- Desktop 人物编辑增加“取消”并丢弃未保存草稿；人物与作品新建/编辑在保存期间锁定整份表单、收起和删除入口，并用同步操作锁拒绝重复提交。收藏、评分、作品首图与人物头像改为按 Private Library 串行读取最新 `PresentationPreference` 后合并字段，避免并发整对象写入互相覆盖。

- Desktop 作品浏览返回状态纳入已有访问历史：从作品卡片或人物入口进入详情前保存完整 WorkQuery、普通分页/瀑布流已加载批次、视图和滚动位置；返回后重新通过 Repository 查询并还原原位置，不缓存 Canonical 实体。

- 按用户要求收口换机交接：补充全流程审核台账、剩余任务优先级、测试边界与换机提示词；明确 JavBus 条目计数不等于已完成去重三语标签库，且本轮未生成新版 EXE/安装包。

- 瀑布流自动预取与手动加载共用同步请求锁，避免同一帧重复触发时连续跳过多个批次。

- 作品编辑新增明确的取消入口：丢弃未保存字段、关系选择和暂存自定义标签并收起表单；收起仍保留草稿，保存行为不变。

- 作品编辑的演员/导演候选支持所有人物姓名、别名、旧艺名搜索，并统一全半角规范化；显示主名和稳定人物关系不变。

- 作品/人物详情在加载、读取失败或记录不存在时保留返回入口，统一使用基础空状态组件；人物详情活动状态与卡片、筛选器共用三语展示映射。

- 普通/成功 Toast 在 4.5 秒展示周期末尾增加真正的淡出动画，相同消息重复触发会重新计时；尊重系统减少动态效果设置，错误和警告仍保留。

- 首页最近作品补齐题材/标签的本地化名称与人物、分类点击导航，与作品库共用卡片及 WorkQuery；收藏和评分读写失败时显示明确错误反馈，不再静默失败。

- 关于页新增离线中日英帮助教程，覆盖首次配置、同步、浏览、编辑、备份、日志和源码/安装版更新路径；明确自动检查更新尚不可用。统一对话框增加屏幕高度上限与正文滚动，保留底部操作可见。

- 全流程审核第一轮：作品/人物详情以资料库路径和实体 ID 隔离组件状态，避免关系跳转复用旧编辑草稿；标签管理使用同步写入锁阻止分类排序与标签编辑并发覆盖，归属检查改为一次读取私人标签集合。
...
- Desktop Work / Person 详情改为按真实访问路径返回：从作品打开人物后，“返回上一页”会回到原作品，关系链可继续逐级回退；只有切换全局一级页面时才清空历史。
...
- Desktop 新增独立“关于”一级入口并固定在窄栏底部，采用 115-Desktop 式单列设置行展示版本、运行环境、程序日志、设置文件位置和项目说明；设置顶部的“关于与诊断”收敛为“诊断”，避免双入口重复。新增可复用 `SettingsRow` UI Primitive，页面只呈现当前真实可用能力。
- Desktop 默认窄侧栏进一步对齐 115-Desktop：宽度收紧为 68px，导航入口统一采用图标在上、短文字在下，选中项使用浅色底、蓝色图标和左侧状态条；设置固定在功能区底部。窄栏不再隐藏文字，也不要求用户依赖 Tooltip 才能理解图标。
- Desktop 建立第一批轻量 UI 基础层：引入 `@fluentui/react-icons` 和 Radix Tooltip，五个全局入口与侧栏展开按钮使用统一 Fluent 图标，窄栏悬停/键盘提示使用统一 Primitive；移除展开状态下残留的旧二级导航，顶部标签成为唯一子分类入口。
- Desktop 导航按 115-Desktop / Fluent 桌面应用的层级继续统一：默认使用 72px 窄侧栏承载五个全局功能，资料库、资料维护和设置的并列页面全部进入同一条顶部标签栏，左侧不再展开另一套子菜单。侧栏资料库卡片压缩为切换器，设置首页将目录概念说明和 Private 存储位置折叠，默认突出资料库与内容目录；资料包导入、导出和备份并入设置顶部导航。
- Desktop “导入与整理”改为单页连续工作台：资料来源摘要、统一目录同步、媒体扫描、NFO / 图片预览导入与 Evidence 核对按实际处理顺序纵向排列。保存 NFO Evidence 后原地刷新下方核对区，无需在扫描与审核页面之间来回跳转；Evidence、Commit Plan、Snapshot 和 Provenance 的执行语义保持不变。
- Desktop 设置拆为资料库与目录、共享资料、扫描与工具、关于与诊断四个模块；内容目录和兼容目录增删后立即持久化。统一同步新增固定阶段反馈，作品筛选新增支持单选或多选的内容目录 Facet，目录归属从 Private `MediaFile.scanRoot` 派生，不污染 Canonical Work。
- “导入与整理”新增 JavBoss 式目录同步卡片：已添加目录始终可见，并显示媒体数量、关联状态和上次扫描；每个目录可以单独执行完整增量同步。目录新增与移除统一收口到设置，工作台不再提供重复入口；单目录清理严格限制在本轮成功扫描根，其他目录记录保持不变。
- Desktop 主导航按日常任务收敛为五个一级区域：首页、资料库、导入与整理、资料维护、设置；作品、人物、分类浏览、收藏、扫描与同步、资料核对、资料问题、变更历史、资料来源与备份作为当前区域的二级入口按需展开。现有页面、查询与 Native 写入边界不变，顶栏同步使用面向用户的页面名称，并补齐日文、英文文案。
- 修正桌面作品浏览的紧凑布局：吸顶筛选栏避让顶部应用栏，窄屏工具栏换行时自动取消二次吸顶；海报卡片的详情入口移到右下角，不再与右上角收藏按钮重叠；作品类型紧跟番号显示；列表收藏按钮并入番号行，不再产生单独空行。
- 网页端 Works 补齐 4K / 1080P / 720P / SD 清晰度筛选：URL 参数 `resolution` 经过受控枚举解析后进入共享 `WorkQuery`，筛选表单、已选条件 Chips 与 self-excluding Facet 计数使用同一结果；同一作品存在多个画质版本时可同时命中多个档位。
- 桌面端筛选面板重构为顶部吸顶筛选栏 + 可折叠抽屉：移除占据整列的左侧 `desktop-facet-panel`，改为整行吸顶的 `desktop-facet-bar`（半透明毛玻璃），主栏常驻搜索 / 排序 / 仅看收藏 / 评分至少 / 视图切换 / 清除 / 「更多筛选」开关；日期区间、时长区间、有封面、有媒体及演员、导演、年份、清晰度、类型、厂商、厂牌、系列、题材、标签等 Facet 组收进抽屉，以自适应卡片网格平铺，带激活数量徽标，首次进入若已选高级筛选则自动展开。重构后海报墙恢复整列宽度，窄屏与暗黑模式同步适配，V1-03 响应式约束（不产生侧栏横向滚动）继续满足。
...
- Desktop 自动记录统一状态与错误日志，固定保存在 App Local Data，单文件达到 1 MiB 后保留三个轮转分卷；配置路径和用户主目录写入前会脱敏，设置页可一键定位日志。
...
- 新建 Library Profile 时由 Native 自动创建独立的受控 Private Library，不再要求用户手选写入目录，也不会继承上一资料库的内容；内容根目录继续由用户按实际影片位置选择。
- NFO / 图片导入结果与预览表格增加明确间距；NFO 未映射词改称“需要留意的导入结果”，并说明它不是持久化应用日志。
- 仓库根目录增加双击式 Windows 构建脚本：分别生成裸 EXE 与 NSIS 安装程序，并自动执行环境/发布预检、保留失败退出码和显示产物位置。
...
- 删除 Library Profile 默认仍只移除配置；对 `App Local Data/libraries/<profile-id>` 下的 Native-managed Private Library 提供第二次明确确认，可选择永久删除管理数据，且删除前拒绝仍被引用、路径不匹配或含链接/Reparse Point 的目录树。
- Work Detail 的 Maker、Label、Series、Work Type、Genre 与 Tag 改为可点击筛选入口，返回统一 Works Explorer 并携带对应 WorkQuery；不创建详情页专属过滤规则。
...
- 空白 Desktop 增加一次选择式首次设置：用户只选择影片资料目录，Native 在 App Local Data 创建受控 Private Library，应用自动建立“我的资料库” Profile；完整路径和 Shared Pack 配置继续保留在高级设置。
- 首页增加“一键同步资料库”主操作，并显示未关联媒体数量与处理入口；同步令牌交给既有 Media 编排执行，避免首页复制 NFO → Asset → Media 业务链。
- Presentation Workbench 的首次加载与保存后刷新统一复用同一快照查询，消除四集合并行读取的重复实现和后续漂移风险。
...
- 修复 Desktop 自绘标题栏未授权导致的窗口按钮和拖动失效；搜索框缩窄并以窗口为基准居中，作品/人物筛选栏紧贴标题栏吸顶。分页底栏在单页时只显示右侧每页数量，多页时再显示左侧翻页、页码和跳转。
- Desktop 影片库从悬浮叠层改为 115-Desktop 式分区布局：筛选工具栏、结果信息、内部滚动视口和分页底栏各自占据独立区域；分页不再覆盖侧栏或卡片，作品与人物页共用同一骨架，海报网格降低列密度以增强图片可读性。
- Desktop 作品/人物页移除说明型大页头，新建 Work/Person 统一并入浏览工具栏并使用弹窗；人物页复用作品页的 Library Layout。分类浏览改用固定搜索工具区和独立内容滚动区，题材分组导航固定在该滚动区顶部。
- 修复 Desktop 人物筛选残留的 sticky 顶部偏移造成整行空白；常用桌面宽度下搜索、状态、排序、更多和新建保持紧凑单行，窄窗口才响应式换行。
- 修复 Desktop 人物搜索框在隐藏字段标题后成为无说明空框的问题，补充“搜索姓名 / 别名 / 旧艺名”占位文字与无障碍名称。
- Desktop 窗口搜索改为页面上下文能力：作品、人物、分类浏览和收藏分别使用对应搜索语义并驱动各自查询，人物与分类正文删除重复搜索框；无明确搜索对象的页面不显示搜索框。
- Desktop 主导航参考 115-Desktop 改为紧凑任务侧栏：作品、人物、分类浏览、目录与扫描成为直接入口，内容顶部不再重复显示影片库四标签；资料维护和设置保留任务内部二级导航。
