...
- 新增 JavBoss 本地仓库对照审查：从截图、README 与实现中归纳渐进瀑布流、紧凑快捷动作、截图灯箱、目录启停和扫描摘要等可吸收体验，明确 GPL-3.0 只参考产品原理，以及在线刮削、播放器、定时任务、文件整理和远程模式与 Localogue 当前 Canonical / Evidence / 非破坏性边界的取舍；形成按日常浏览、媒体动作、资料源管理、扫描反馈逐步推进的实施顺序。
- 桌面瀑布流改为真正的渐进式无限滚动：Repository 继续按页查询，接近页尾时由 `IntersectionObserver` 提前加载并按作品 ID 去重追加，另保留键盘可操作的“加载更多”按钮；移除一次读取 100000 条作品的临时方案，降低大型资料库的首次等待、内存与 DOM 压力。作品详情返回到瀑布流深处时会按需补齐页面高度后再恢复原滚动位置。
- 修复桌面作品分页被旧异步结果立即重置的问题；页码校正只处理与当前请求匹配的完成结果。瀑布流改为一次连续展示全部匹配作品、不显示分页器，并提供小 / 中 / 大三档持久化图片宽度；其它视图继续分页，避免普通列表创建过多 DOM 节点。
- 桌面作品浏览器记住当前会话中的筛选、页码与进入详情前的滚动位置，从作品详情返回后继续原位置浏览；资料库切换或结果缩减时，组件会采用共享查询返回的实际有效页码，直接落到最后一页，避免在超出总页数的空页之间反复向前翻。
