/**
 * Browser / Node neutral SHA-256 for small deterministic governance payloads.
 *
 * This intentionally avoids node:crypto so Review Commit Plan can run inside
 * the Tauri WebView as well as Next.js. It accepts UTF-8 text and returns a
...
export function sha256Text(input: string): string {
  ...
  for (let offset = 0; offset < padded.length; offset += 64) {
    ...
    for (let index = 0; index < 64; index += 1) {
      ...
      const temp1 = add(h, s1, choice, k[index], words[index]);
      const s0 = rotr(a, 2) ^ rotr(a, 13) ^ rotr(a, 22);
      const majority = (a & b) ^ (a & c) ^ (b & c);
      const temp2 = add(s0, majority);
