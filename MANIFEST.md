...
**V2 Storage Migration：Evidence / Curated Catalog / Personal Library + SQLite。** 已建立标准 SQLite `catalog.db` Schema、Community JSON 原子构建器和逐集合/完整性/外键/Crosswalk 对账；Shared Pack 统一解释为 Curated Catalog 的只读发布包。运行时默认 Repository 暂未切换，待 Contract Test 通过后再启用。
