...
import { activeLibraryProfile, updateLibraryProfile } from "./library-profiles";
...
export function DesktopPacksPage({
  ...
  async function addPack(): Promise<void> {
    ...
    try {
      ...
      setSettings((current) => {
        const active = activeLibraryProfile(current);
        return active ? updateLibraryProfile(current, active.id, { sharedPackPaths: unique([...active.sharedPackPaths, path]) }) : current;
      });
  ...
  function movePack(index: number, direction: -1 | 1): void {
    setSettings((current) => {
      ...
      const next = [...active.sharedPackPaths];
      const target = index + direction;
      if (target < 0 || target >= next.length) return current;
      [next[index], next[target]] = [next[target], next[index]];
      return updateLibraryProfile(current, active.id, { sharedPackPaths: next });
    });
  }
  ...
  function removePack(path: string): void {
    setSettings((current) => {
      const active = activeLibraryProfile(current);
      return active ? updateLibraryProfile(current, active.id, { sharedPackPaths: active.sharedPackPaths.filter((item) => item !== path) }) : current;
    });
  }
  ...
  const sharedPackPaths = profile?.sharedPackPaths ?? [];
  const hasDraftChanges = JSON.stringify(sharedPackPaths) !== JSON.stringify(packInfos.map((item) => item.configuredPath));
