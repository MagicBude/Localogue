...
const guides: Record<SupportedLanguage, { title: string; introduction: string; update: string; updateStatus: string; steps: Array<[string, string]> }> = {
  "zh-CN": {
    title: "帮助与使用教程", introduction: "从添加目录到浏览、编辑和备份。", update: "版本与更新", updateStatus: "当前版本不提供自动检查或安装更新。更新前请先备份影片库。",
    steps: [
      ["1. 创建影片库", "首次启动创建影片库并选择内容目录，或到设置 → 影片库与目录继续管理。内容目录保存原始视频、NFO 和图片；数据存储位置保存 Localogue 管理的作品资料、收藏和个人修改。"],
      ["2. 扫描与整理", "在导入与整理中扫描整个影片库或某个内容目录。扫描依次处理 NFO、图片和视频；高级预览可以在导入前核对内容。未识别的视频可以之后手动关联作品。"],
      ...
      ["5. 社区资料与备份", "设置 → 社区资料可以安装只读的社区资料包，也可以导入或导出个人备份。个人备份不包含原始视频，应另外备份视频和本机目录设置。关于 → 程序日志可查看失败原因。"],
