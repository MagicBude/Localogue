...
- 应用内查看同样只能读取这一个固定日志文件，WebView 不能传入任意路径；读取也在 blocking worker 中执行。

“关于”页提供应用内日志弹窗，默认渲染当前文件最近 1,000 行，并支持级别、关键词筛选和刷新。弹窗保留“打开文件所在位置”，用户需要删除或管理轮转文件时交给系统文件管理器处理；Localogue 不在日志查看器中增加无确认的删除操作。
