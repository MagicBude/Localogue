...
当前最新导航节点：详情页只保留右下角悬浮返回；作品与人物分页共用 `desktop-pagination.tsx`，停靠在窗口底部固定底栏。顶部应用框架使用自绘无边框标题栏，搜索、语言和窗口控制都固定在窗口最上方，删除重复设置与手动刷新。继续开发时不要重做分页或浏览快照，先验收长页面中段返回、底栏翻页、搜索、翻页后定位和多级关系返回。
...
收藏现为左侧独立入口，不要再同时放回资料库顶部或“更多筛选”。普通筛选只暴露目录、人物、作品类型/题材/标签、年份、时长、评分、本地媒体和清晰度；厂商、厂牌、系列、精确日期和封面状态仍由 WorkQuery 兼容，主要从分类浏览、详情反向导航和已有查询状态进入。

作品海报墙已按 JavBoss 的横向封面卡片思路重做：横向 `cover` 优先、标题两行、人物最多 3 位、分类最多 6 项，瀑布流继续承担竖版连续浏览。当前开发资料库实测 46 个 Work、0 个 Asset，所以占位图不能靠 CSS 解决；下一步应在不破坏 Unified Library 同步链的前提下设计封面补齐入口。
