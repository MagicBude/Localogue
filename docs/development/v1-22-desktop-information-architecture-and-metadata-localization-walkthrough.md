...
- 顶部：全宽媒体画廊，按 poster → fanart → thumbnail → cover → 其他图片排序；
- 画廊：左右箭头切换当前图片，底部轻量标签用于直接跳转，不一次性加载全部缩略图；
- 下方：番号、标题、简介与全宽紧凑字段表；
- 字段表：发行日期、时长、演员、导演、Maker、Label、Series、作品类型、题材、标签；
- 更下方：折叠编辑器和紧凑本地 Asset 管理列表。

分类不再放到页面最底部；顶部画廊和下方全宽信息表也避免固定左图在长 Metadata 页面中产生大块纵向空白。
